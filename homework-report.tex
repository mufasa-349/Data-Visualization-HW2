...
\begin{abstract}
This report describes Task~1 of the homework: building an interactive Python dashboard for Japan inbound immigration statistics, analyzing the interface with Gestalt and Bertin principles, and documenting revisions made to improve visual encoding and readability. Screenshots of the dashboard \emph{before} and \emph{after} revision are included, together with a structured comparison of analytical findings and implemented changes. Program code for the dashboard and configuration was produced with assistance from \textbf{Cursor}, used as the AI coding agent in the development workflow.
\end{abstract}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Outcome:} The report below merges that analysis with a \textbf{before/after} narrative: Figure~\ref{fig:before} reflects the pre-revision UI; Section~\ref{sec:revision} summarizes changes aligned with the checklist; Figure~\ref{fig:after} shows the revised dashboard.
\end{itemize}
...
\section{Before vs.\ after --- concise comparison}
\label{sec:compare}

\begin{itemize}[leftmargin=*]
  \item \textbf{Gestalt:} Grouping of controls and charts was already good; revisions strengthened \textbf{similarity} between Streamlit accents and chart data (config primary color) and improved \textbf{closure} for the total trend (area fill). \textbf{Proximity} of meaning was improved through KPI help text and chart subtitles rather than by changing the four-column KPI layout.
  \item \textbf{Bertin:} \textbf{Position} was left correct; \textbf{redundant encoding} was added for YoY (hue plus vertical position); \textbf{color value} and tick contrast were systematized; line \textbf{mark} noise was reduced. Units and field definitions are now explicit in labels, subtitles, and tooltips.
\end{itemize}

\section{Conclusion}

We built an interactive Japan inbound immigration dashboard in Python (Streamlit + Plotly), analyzed it with Gestalt and Bertin lenses, and applied a focused set of revisions: thematic consistency, redundant sign encoding for YoY, improved typography/contrast, area fill for the total series, and clearer documentation of metrics directly in the UI. The screenshots \texttt{task1-a.png} and \texttt{task1-b.png} document the interface before and after these changes.
